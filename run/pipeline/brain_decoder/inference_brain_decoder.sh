...
MODELS=('NeuroSketch' 'ModernTCN' 'MedFormer' 'MultiResGRU')  # Models to use
SEEDS=($(seq 42 51))
  ...
  for task in "${TASKS[@]}"; do
    ...
    for model in "${MODELS[@]}"; do
      for seed in "${SEEDS[@]}"; do
        param_combinations+=("$id $task $channels_str $model $seed")
      done
    done
  done
done
...
for ((batch=0; batch<total_batches; batch++)); do
    ...
    for ((i=start; i<end && i<total_jobs; i++)); do
        IFS=' ' read -r id task channels_str model seed<<< "${param_combinations[$i]}"
        ...
        commands+=("CUDA_VISIBLE_DEVICES=$gpu_id python $PROJECT_DIR/run/train.py \\
            ...
            swanlab.exp_name=\"${model}_${id}_${task}_inference_${seed}\"")

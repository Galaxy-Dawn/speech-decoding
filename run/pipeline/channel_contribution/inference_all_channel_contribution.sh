  ...
  for task in "${TASKS[@]}"; do
    for channel in "${channels_array[@]}"; do
      for seed in "${SEEDS[@]}"; do
        param_combinations+=("$subject $task $channel $seed")
      done
    done
  done
done
...
for ((batch=0; batch<total_batches; batch++)); do
    ...
    for ((i=start; i<end && i<total_jobs; i++)); do
      IFS=' ' read -r subject task channel seed<<< "${param_combinations[$i]}"
      gpu_id=$(( (i % total_gpus)))
      commands+=("CUDA_VISIBLE_DEVICES=$gpu_id python ${PROJECT_DIR}/run/train.py \\
          ...
          swanlab.exp_name=\"${subject}_channel_contribution_${task}_$(echo $channel | tr '-' '_')_inference_$seed\"")

...
run_correction_inference() {
    ...
    MODEL_NAME=$(yq e '.correction.model_name' "$CONFIG_FILE")
    DATASET_NAME=$(yq e '.correction.dataset_name' "$CONFIG_FILE")
    MODEL_PATH=$(yq e '.correction.model_path' "$CONFIG_FILE")
    OUTPUT_PATH=$(yq e '.correction.output_path' "$CONFIG_FILE")

    local MODEL_PATH=${MODEL_PATH/\$\{inference_module.correction.model_name\}/$MODEL_NAME}
    ...
    local OUTPUT_PATH=${OUTPUT_PATH/\$\{inference_module.correction.model_name\}/$MODEL_NAME}
